...
function buildIndexMapping(map) {
  var field = {};
  field["sort_field_" + map.field] = {
    "match": map.field,
    "match_pattern": 'regex',
...
var buildNewIndex = function buildNewIndex(index) {
  ...
  if (map.hasOwnProperty('sorts')) {
    ...
    for (var i in map.sorts) {
      body.mappings._default_.dynamic_templates.push(buildIndexMapping(map.sorts[i]));
    }
  }

  // Setup char based ngram search fields.
  if (map.hasOwnProperty('char_search')) {
